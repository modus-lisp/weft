;;;; src/css/color.lisp — CSS <color> value parser
(in-package #:weft.css)

;;;; ---- CSS Color 4/5: lab()/lch()/oklab()/oklch() and color-mix() ----------
;;;; Modern frameworks (Tailwind v4 / turbopack) emit theme colors as lab()/
;;;; oklch() with a hex fallback gated behind @supports.  weft flattens @supports
;;;; (assumes supported), so the modern form wins the cascade and MUST resolve or
;;;; every themed color collapses to unset.  We convert to sRGB here.

(defun %srgb-encode (c)
  "Linear-light sRGB component (0..1, may be out of gamut) -> gamma-encoded 0..255."
  (let* ((c (max 0.0 (min 1.0 c)))
         (v (if (<= c 0.0031308)
                (* 12.92 c)
                (- (* 1.055 (expt c (/ 1.0 2.4))) 0.055))))
    (max 0 (min 255 (round (* v 255.0))))))

(defun %lin-srgb->list (r g b a)
  (list (%srgb-encode r) (%srgb-encode g) (%srgb-encode b) (float (max 0.0 (min 1.0 a)))))

(defun %oklab->srgb (l a b alpha)
  "OKLab (L 0..1, a,b) -> (r g b a) sRGB.  Björn Ottosson's OKLab matrices."
  (let* ((l_ (+ l (* 0.3963377774 a) (* 0.2158037573 b)))
         (m_ (+ l (* -0.1055613458 a) (* -0.0638541728 b)))
         (s_ (+ l (* -0.0894841775 a) (* -1.2914855480 b)))
         (ll (* l_ l_ l_)) (mm (* m_ m_ m_)) (ss (* s_ s_ s_))
         (r (+ (* 4.0767416621 ll) (* -3.3077115913 mm) (* 0.2309699292 ss)))
         (g (+ (* -1.2684380046 ll) (* 2.6097574011 mm) (* -0.3413193965 ss)))
         (bl (+ (* -0.0041960863 ll) (* -0.7034186147 mm) (* 1.7076147010 ss))))
    (%lin-srgb->list r g bl alpha)))

(defun %lab->srgb (l a b alpha)
  "CIE Lab (D50, L 0..100) -> (r g b a) sRGB, per CSS Color 4."
  (let* ((kappa 903.2962962962963) (eps 0.008856451679035631)
         (fy (/ (+ l 16.0) 116.0))
         (fx (+ fy (/ a 500.0)))
         (fz (- fy (/ b 200.0)))
         (fx3 (* fx fx fx)) (fz3 (* fz fz fz))
         (xr (if (> fx3 eps) fx3 (/ (- (* 116.0 fx) 16.0) kappa)))
         (yr (if (> l (* kappa eps)) (let ((q (/ (+ l 16.0) 116.0))) (* q q q)) (/ l kappa)))
         (zr (if (> fz3 eps) fz3 (/ (- (* 116.0 fz) 16.0) kappa)))
         ;; D50 white point
         (x (* xr 0.9642956764295677)) (y (* yr 1.0)) (z (* zr 0.8251046025104602))
         ;; Bradford chromatic adaptation D50 -> D65
         (x65 (+ (* 0.9554734527042182 x) (* -0.023098536874261423 y) (* 0.0632593086610217 z)))
         (y65 (+ (* -0.028369706963208136 x) (* 1.0099954580058226 y) (* 0.021041398966943008 z)))
         (z65 (+ (* 0.012314001688319899 x) (* -0.020507696433477912 y) (* 1.3303659366080753 z)))
         ;; XYZ (D65) -> linear sRGB
         (r (+ (* 3.2409699419045226 x65) (* -1.537383177570094 y65) (* -0.4986107602930034 z65)))
         (g (+ (* -0.9692436362808796 x65) (* 1.8759675015077202 y65) (* 0.04155505740717559 z65)))
         (bl (+ (* 0.05563007969699366 x65) (* -0.20397695888897652 y65) (* 1.0569715142428786 z65))))
    (%lin-srgb->list r g bl alpha)))

(defun %split-top-char (s ch)
  "Split S on CH at top nesting level (ignoring CH inside parens)."
  (let ((out '()) (depth 0) (start 0) (n (length s)))
    (dotimes (i n)
      (let ((c (char s i)))
        (cond ((char= c #\() (incf depth))
              ((char= c #\)) (when (plusp depth) (decf depth)))
              ((and (char= c ch) (zerop depth))
               (push (subseq s start i) out) (setf start (1+ i))))))
    (push (subseq s start n) out)
    (nreverse out)))

(defun %color-fn-interior (s open-kw)
  "Interior of `KW( ... )` in S (KW length via OPEN-KW), auto-closing at EOF."
  (let* ((open (position #\( s))
         (close (and open (matching-paren s open))))
    (declare (ignore open-kw))
    (and open (subseq s (1+ open) (or close (length s))))))

(defun %split-color-components (interior)
  "Split a modern color-function INTERIOR into (values main-list alpha-str).
Components are whitespace/comma separated; an alpha follows a top-level `/`."
  (let* ((slash (let ((depth 0) (pos nil))
                  (dotimes (i (length interior))
                    (case (char interior i)
                      (#\( (incf depth)) (#\) (when (plusp depth) (decf depth)))
                      (#\/ (when (and (zerop depth) (null pos)) (setf pos i)))))
                  pos))
         (main (if slash (subseq interior 0 slash) interior))
         (alpha (and slash (css-trim (subseq interior (1+ slash))))))
    ;; split main on whitespace/commas at top level
    (let ((out '()) (depth 0) (start nil) (n (length main)))
      (dotimes (i n)
        (let ((c (char main i)))
          (cond ((char= c #\() (incf depth) (unless start (setf start i)))
                ((char= c #\)) (when (plusp depth) (decf depth)))
                ((and (zerop depth) (member c '(#\Space #\Tab #\Newline #\,)))
                 (when start (push (subseq main start i) out) (setf start nil)))
                (t (unless start (setf start i))))))
      (when start (push (subseq main start n) out))
      (values (nreverse out) alpha))))

(defun %num-or-pct (str pct100 &optional (default 0.0))
  "Parse a modern-color component STR: a <number>, a <percentage> where 100% maps
to PCT100, or `none` (-> DEFAULT).  NIL on failure."
  (let ((str (css-trim str)))
    (cond ((zerop (length str)) nil)
          ((string-equal str "none") default)
          ((char= (char str (1- (length str))) #\%)
           (let ((v (safe-number (subseq str 0 (1- (length str))))))
             (and v (* (/ v 100.0) pct100))))
          (t (safe-number str)))))

(defun %hue-deg (str)
  "Parse a hue component (number in degrees, or angle unit) -> degrees float."
  (let ((str (css-trim str)))
    (cond ((zerop (length str)) 0.0)
          ((string-equal str "none") 0.0)
          ((angle-tok-p str) (let ((a (parse-value "angle" str)))
                               (if (consp a) (angle->deg a) 0.0)))
          (t (or (safe-number str) 0.0)))))

(defun %parse-alpha-comp (str)
  "Parse an alpha component -> 0..1, default 1.0 when NIL."
  (if (null str) 1.0
      (let ((str (css-trim str)))
        (cond ((zerop (length str)) 1.0)
              ((string-equal str "none") 1.0)
              ((char= (char str (1- (length str))) #\%)
               (let ((v (safe-number (subseq str 0 (1- (length str)))))) (if v (/ v 100.0) 1.0)))
              (t (or (safe-number str) 1.0))))))

(defun parse-lab-like (s kind)
  "Parse lab()/lch()/oklab()/oklch() S (trimmed, downcased) -> (r g b a) or :invalid.
KIND is one of :lab :lch :oklab :oklch."
  (let ((interior (%color-fn-interior s nil)))
    (unless interior (return-from parse-lab-like :invalid))
    (multiple-value-bind (comps alpha-str) (%split-color-components interior)
      (unless (>= (length comps) 3) (return-from parse-lab-like :invalid))
      (let ((a (%parse-alpha-comp alpha-str)))
        (ecase kind
          (:oklab (let ((l (%num-or-pct (first comps) 1.0))
                        (aa (%num-or-pct (second comps) 0.4))
                        (bb (%num-or-pct (third comps) 0.4)))
                    (if (and l aa bb) (%oklab->srgb l aa bb a) :invalid)))
          (:lab (let ((l (%num-or-pct (first comps) 100.0))
                      (aa (%num-or-pct (second comps) 125.0))
                      (bb (%num-or-pct (third comps) 125.0)))
                  (if (and l aa bb) (%lab->srgb l aa bb a) :invalid)))
          (:oklch (let ((l (%num-or-pct (first comps) 1.0))
                        (c (%num-or-pct (second comps) 0.4))
                        (h (%hue-deg (third comps))))
                    (if (and l c) (let ((rad (* h (/ (float pi 1.0) 180.0))))
                                    (%oklab->srgb l (* c (cos rad)) (* c (sin rad)) a)) :invalid)))
          (:lch (let ((l (%num-or-pct (first comps) 100.0))
                      (c (%num-or-pct (second comps) 150.0))
                      (h (%hue-deg (third comps))))
                  (if (and l c) (let ((rad (* h (/ (float pi 1.0) 180.0))))
                                  (%lab->srgb l (* c (cos rad)) (* c (sin rad)) a)) :invalid))))))))

(defun parse-color-mix (s)
  "Parse color-mix(in <space>, c1 [p1], c2 [p2]) (CSS Color 5) -> (r g b a).
Mixes in premultiplied sRGB (a pragmatic first cut; the dominant real-world use —
`X p%, transparent` — is colorspace-independent).  :invalid on failure."
  (let ((interior (%color-fn-interior s nil)))
    (unless interior (return-from parse-color-mix :invalid))
    (let ((args (%split-top-char interior #\,)))
      (unless (>= (length args) 3) (return-from parse-color-mix :invalid))
      ;; args[0] = "in <space...>"; ignore the interpolation space (we mix in sRGB).
      (flet ((split-color-pct (arg)
               ;; "<color> [<percentage>]" -> (values color-string pct-or-nil)
               (multiple-value-bind (toks alpha) (%split-color-components arg)
                 (declare (ignore alpha))
                 (let* ((last (car (last toks)))
                        (pct (and last (plusp (length last))
                                  (char= (char last (1- (length last))) #\%)
                                  (safe-number (subseq last 0 (1- (length last)))))))
                   (if pct
                       (values (format nil "~{~a~^ ~}" (butlast toks)) pct)
                       (values (css-trim arg) nil))))))
        (multiple-value-bind (c1s p1) (split-color-pct (second args))
          (multiple-value-bind (c2s p2) (split-color-pct (third args))
            (let ((c1 (parse-value "color" c1s))
                  (c2 (parse-value "color" c2s)))
              (when (or (not (listp c1)) (not (listp c2)) (< (length c1) 4) (< (length c2) 4))
                (return-from parse-color-mix :invalid))
              ;; Resolve percentages per CSS Color 5 §2.1.
              (let* ((both (and p1 p2))
                     (p1n (cond (both p1) (p1 p1) (p2 (- 100.0 p2)) (t 50.0)))
                     (p2n (cond (both p2) (p1 (- 100.0 p1)) (p2 p2) (t 50.0)))
                     (sum (+ p1n p2n)))
                (when (<= sum 0.0) (return-from parse-color-mix :invalid))
                (let* ((alpha-mult (if (< sum 100.0) (/ sum 100.0) 1.0))
                       (w1 (/ p1n sum)) (w2 (/ p2n sum))
                       (a1 (float (fourth c1))) (a2 (float (fourth c2)))
                       ;; premultiplied mixing
                       (pa (+ (* a1 w1) (* a2 w2)))
                       (mix (lambda (i)
                              (let ((v (+ (* (nth i c1) a1 w1) (* (nth i c2) a2 w2))))
                                (if (> pa 0.0) (/ v pa) 0.0))))
                       (out-a (* pa alpha-mult)))
                  (list (max 0 (min 255 (round (funcall mix 0))))
                        (max 0 (min 255 (round (funcall mix 1))))
                        (max 0 (min 255 (round (funcall mix 2))))
                        (float (max 0.0 (min 1.0 out-a)))))))))))))

;;;; ---- CSS Color 5 relative color: rgb(from C ...) / hsl(from C ...) ---------
(defun %hsl-to-rgb (h s l)
  "H in degrees, S/L in 0-100 -> (values R G B) each 0-255 (CSS Color 4 §7)."
  (let* ((s (/ s 100.0)) (l (/ l 100.0))
         (c (* (- 1.0 (abs (- (* 2.0 l) 1.0))) s))
         (x (* c (- 1.0 (abs (- (mod (/ h 60.0) 2.0) 1.0)))))
         (m (- l (/ c 2.0)))
         (h-sec (floor h 60)))
    (multiple-value-bind (r1 g1 b1)
        (case h-sec (0 (values c x 0)) (1 (values x c 0)) (2 (values 0 c x))
              (3 (values 0 x c)) (4 (values x 0 c)) (5 (values c 0 x)) (t (values 0 0 0)))
      (values (round (* (+ r1 m) 255)) (round (* (+ g1 m) 255)) (round (* (+ b1 m) 255))))))

(defun %rgb-to-hsl (r g b)
  "R G B in 0-255 -> (values H[0-360] S[0-100] L[0-100])."
  (let* ((rf (/ r 255.0)) (gf (/ g 255.0)) (bf (/ b 255.0))
         (mx (max rf gf bf)) (mn (min rf gf bf)) (d (- mx mn))
         (l (/ (+ mx mn) 2.0))
         (s (if (zerop d) 0.0 (/ d (- 1.0 (abs (- (* 2.0 l) 1.0))))))
         (h (cond ((zerop d) 0.0)
                  ((= mx rf) (* 60.0 (mod (/ (- gf bf) d) 6.0)))
                  ((= mx gf) (* 60.0 (+ (/ (- bf rf) d) 2.0)))
                  (t (* 60.0 (+ (/ (- rf gf) d) 4.0))))))
    (values (mod h 360.0) (* 100.0 s) (* 100.0 l))))

(defun %rc-tokenize (s)
  "Tokenize an arithmetic expression S into (:num . float) | op-char | paren-char |
identifier-string tokens.  NIL on an unexpected character."
  (let ((out '()) (i 0) (n (length s)))
    (loop while (< i n) do
      (let ((c (char s i)))
        (cond ((member c '(#\Space #\Tab #\Newline #\Return)) (incf i))
              ((member c '(#\+ #\- #\* #\/ #\( #\))) (push c out) (incf i))
              ((or (digit-char-p c) (char= c #\.))
               (let ((j i))
                 (loop while (and (< j n) (or (digit-char-p (char s j)) (char= (char s j) #\.))) do (incf j))
                 (let ((v (safe-number (subseq s i j)))) (unless v (return-from %rc-tokenize nil))
                   (push (cons :num (float v 1.0)) out)) (setf i j)))
              ((alpha-char-p c)
               (let ((j i))
                 (loop while (and (< j n) (alpha-char-p (char s j))) do (incf j))
                 (push (subseq s i j) out) (setf i j)))
              (t (return-from %rc-tokenize nil)))))
    (nreverse out)))

(defun %rc-eval (s vars)
  "Evaluate arithmetic expression string S (channel names looked up in VARS, an alist
name->float) via shunting-yard -> float | NIL."
  (let ((toks (%rc-tokenize s)))
    (unless toks (return-from %rc-eval nil))
    (let ((output '()) (ops '()))
      (labels ((prec (o) (if (member o '(#\* #\/)) 2 1))
               (apply-op ()
                 (let ((o (pop ops)) (b (pop output)) (a (pop output)))
                   (unless (and (numberp a) (numberp b)) (return-from %rc-eval nil))
                   (push (case o (#\+ (+ a b)) (#\- (- a b)) (#\* (* a b))
                               (#\/ (if (zerop b) (return-from %rc-eval nil) (/ a b))))
                         output))))
        (dolist (tk toks)
          (cond ((and (consp tk) (eq (car tk) :num)) (push (cdr tk) output))
                ((stringp tk) (let ((cell (assoc tk vars :test #'string-equal)))
                                (unless cell (return-from %rc-eval nil))
                                (push (float (cdr cell) 1.0) output)))
                ((eql tk #\() (push tk ops))
                ((eql tk #\)) (loop while (and ops (not (eql (car ops) #\())) do (apply-op))
                              (if (and ops (eql (car ops) #\()) (pop ops) (return-from %rc-eval nil)))
                (t (loop while (and ops (characterp (car ops)) (not (eql (car ops) #\())
                                    (>= (prec (car ops)) (prec tk)))
                         do (apply-op))
                   (push tk ops))))
        (loop while ops do (if (eql (car ops) #\() (return-from %rc-eval nil) (apply-op)))
        (and output (null (cdr output)) (numberp (car output)) (car output))))))

(defun %rc-component (str vars)
  "Evaluate a relative-color component STR (a channel keyword, number, none, or a
calc()) against VARS -> float | NIL."
  (let ((str (css-trim str)))
    (cond ((zerop (length str)) nil)
          ((string-equal str "none") 0.0)
          ((and (>= (length str) 5) (string-equal (subseq str 0 5) "calc("))
           (%rc-eval (subseq str 5 (or (matching-paren str 4) (length str))) vars))
          (t (%rc-eval str vars)))))

(defun parse-relative-color (s kind)
  "Parse a CSS Color 5 relative color rgb(from C r g b [/a]) or hsl(from C h s l [/a])
-> (r g b a).  KIND is :rgb or :hsl.  :invalid on failure."
  (let ((interior (%color-fn-interior s nil)))
    (unless interior (return-from parse-relative-color :invalid))
    (let ((interior (css-trim interior)))
      (unless (and (>= (length interior) 5) (string-equal (subseq interior 0 5) "from "))
        (return-from parse-relative-color :invalid))
      (multiple-value-bind (toks alpha-str) (%split-color-components (subseq interior 5))
        (unless (>= (length toks) 4) (return-from parse-relative-color :invalid))
        (let ((origin (parse-value "color" (first toks))))
          (unless (and (listp origin) (>= (length origin) 4))
            (return-from parse-relative-color :invalid))
          (destructuring-bind (o0 o1 o2 oa) (subseq origin 0 4)
            (let* ((vars (if (eq kind :hsl)
                             (multiple-value-bind (h sv l) (%rgb-to-hsl o0 o1 o2)
                               (list (cons "h" h) (cons "s" sv) (cons "l" l) (cons "alpha" oa)))
                             (list (cons "r" (float o0 1.0)) (cons "g" (float o1 1.0))
                                   (cons "b" (float o2 1.0)) (cons "alpha" oa))))
                   (c0 (%rc-component (nth 1 toks) vars))
                   (c1 (%rc-component (nth 2 toks) vars))
                   (c2 (%rc-component (nth 3 toks) vars))
                   (a (if alpha-str (%rc-component alpha-str vars) oa)))
              (unless (and c0 c1 c2 a) (return-from parse-relative-color :invalid))
              (flet ((cl (v) (max 0 (min 255 (round v)))))
                (if (eq kind :hsl)
                    (multiple-value-bind (r g b)
                        (%hsl-to-rgb (mod c0 360.0) (max 0.0 (min 100.0 c1)) (max 0.0 (min 100.0 c2)))
                      (list (round r) (round g) (round b) (float (max 0.0 (min 1.0 a)) 1.0)))
                    (list (cl c0) (cl c1) (cl c2) (float (max 0.0 (min 1.0 a)) 1.0)))))))))))

(define-value-parser "color" (s)
  (let* ((s (css-trim s))
         (s (ascii-downcase s))
         (len (length s)))
    (labels
        ((digit-val (c)
           (if (digit-char-p c)
               (digit-char-p c)
               (+ 10 (- (char-code c) (char-code #\a)))))
         (hex-digit-p (c)
           (or (digit-char-p c 16)
               (and (char>= c #\a) (char<= c #\f))))
         (unhex2 (str &optional (start 0))
           (+ (* (digit-val (char str start)) 16)
              (digit-val (char str (1+ start)))))
         (nibble-val (c)
           (let ((v (digit-val c)))
             (+ (* v 16) v)))
         (round4 (x)
           (/ (round (* x 10000)) 10000.0))
         (clamp (v lo hi)
           (max lo (min hi v)))
         (try-parse-num (str)
           ;; Only a real number is a valid numeric component; a bare ident like
           ;; `none` reads as a SYMBOL, which must not leak into the arithmetic
           ;; below (CSS Color 4 `none` is unmodeled -> reject the component).
           (let ((v (ignore-errors (let ((*read-eval* nil)) (read-from-string str)))))
             (and (realp v) v)))
         ;; Hex parsing
         (parse-hex ()
           (let* ((body (subseq s 1))
                  (blen (length body)))
             (unless (and (> blen 0) (every #'hex-digit-p body))
               (return-from parse-hex :invalid))
             (case blen
               (3 (list (nibble-val (char body 0))
                        (nibble-val (char body 1))
                        (nibble-val (char body 2))
                        1.0))
               (4 (list (nibble-val (char body 0))
                        (nibble-val (char body 1))
                        (nibble-val (char body 2))
                        (round4 (/ (nibble-val (char body 3)) 255.0))))
               (6 (list (unhex2 body 0)
                        (unhex2 body 2)
                        (unhex2 body 4)
                        1.0))
               (8 (list (unhex2 body 0)
                        (unhex2 body 2)
                        (unhex2 body 4)
                        (round4 (/ (unhex2 body 6) 255.0))))
               (t :invalid))))
         ;; Split comma/slash-separated components
         (split-comps (str)
           (let* ((n (length str))
                  (buf (make-string n)))
             (dotimes (i n)
               (setf (char buf i)
                     (if (or (char= (char str i) #\,)
                             (char= (char str i) #\/))
                         #\Space
                         (char str i))))
             (let ((parts '()) (start nil))
               (loop for i from 0 below n do
                 (cond ((and (null start) (not (char= (char buf i) #\Space)))
                        (setf start i))
                       ((and (not (null start)) (char= (char buf i) #\Space))
                        (push (subseq buf start i) parts)
                        (setf start nil))))
               (when start (push (subseq buf start n) parts))
               (nreverse parts))))
         ;; Parse RGB component: "50%" -> (* 2.55 pct), else number
         (parse-rgb-comp (str)
           (let ((l (length str)))
             (if (and (> l 0) (char= (char str (1- l)) #\%))
                 (let ((v (try-parse-num (subseq str 0 (1- l)))))
                   (when v (* v 2.55)))
                 (try-parse-num str))))
         ;; Parse alpha component: "50%" -> 0.5, else float
         (parse-alpha (str)
           (let ((l (length str)))
             (if (and (> l 0) (char= (char str (1- l)) #\%))
                 (let ((v (try-parse-num (subseq str 0 (1- l)))))
                   (when v (/ v 100.0)))
                 (try-parse-num str))))
         ;; RGB/RGBA parser
         (parse-rgb ()
           ;; A missing close paren is not fatal: at EOF an open function auto-closes
           ;; (CSS Syntax §5.4.9), so `rgb(0, 128, 0` yields the components up to end.
           (let* ((open (position #\( s))
                  (close (when open (position #\) s :start open)))
                  (content (and open (subseq s (1+ open) (or close (length s)))))
                  (parts (and content (split-comps content)))
                  (n (length parts)))
             (unless (or (= n 3) (= n 4))
               (return-from parse-rgb :invalid))
             ;; CSS Color 3: the three R/G/B components must be ALL <percentage>
             ;; or ALL <integer> — mixing them (`rgb(100%, 0, 0)`) is invalid.
             (flet ((pctp (str) (let ((l (length str)))
                                  (and (> l 0) (char= (char str (1- l)) #\%)))))
               (let ((p0 (pctp (elt parts 0))))
                 (unless (and (eq p0 (pctp (elt parts 1))) (eq p0 (pctp (elt parts 2))))
                   (return-from parse-rgb :invalid))))
             (let* ((r (parse-rgb-comp (elt parts 0)))
                    (g (parse-rgb-comp (elt parts 1)))
                    (b (parse-rgb-comp (elt parts 2)))
                    (a (if (= n 4)
                           (let ((av (parse-alpha (elt parts 3))))
                             (when av (clamp (round4 av) 0.0 1.0)))
                           1.0)))
               (if (and r g b a)
                   (list (clamp (round r) 0 255)
                         (clamp (round g) 0 255)
                         (clamp (round b) 0 255)
                         (float a))
                   :invalid))))
         ;; HSL algorithm
         (hsl-to-rgb (h s l)
           (let* ((s (/ s 100.0))
                  (l (/ l 100.0))
                  (c (* (- 1 (abs (- (* 2 l) 1))) s))
                  (x (* c (- 1 (abs (- (mod (/ h 60.0) 2) 1)))))
                  (m (- l (/ c 2)))
                  (h-sec (floor h 60)))
             (multiple-value-bind (r1 g1 b1)
                 (case h-sec
                   (0 (values c x 0))
                   (1 (values x c 0))
                   (2 (values 0 c x))
                   (3 (values 0 x c))
                   (4 (values x 0 c))
                   (5 (values c 0 x))
                   (t (values 0 0 0)))
               (values (round (* (+ r1 m) 255))
                       (round (* (+ g1 m) 255))
                       (round (* (+ b1 m) 255))))))
         ;; HSL/HSLA parser
         (parse-hsl ()
           (let* ((open (position #\( s))
                  (close (when open (position #\) s :start open)))
                  (content (and close (> close open) (subseq s (1+ open) close)))
                  (parts (and content (split-comps content)))
                  (n (length parts)))
             (unless (or (= n 3) (= n 4))
               (return-from parse-hsl :invalid))
             (let* ((h-str (elt parts 0))
                    (h-len (length h-str))
                    ;; Strip trailing "deg" if present
                    (h-clean (if (and (>= h-len 3)
                                      (string= (subseq h-str (- h-len 3)) "deg"))
                                 (subseq h-str 0 (- h-len 3))
                                 h-str))
                    (h-val (try-parse-num h-clean))
                    ;; S and L: must be percentages
                    (s-str (elt parts 1))
                    (l-str (elt parts 2))
                    (s-val (and (> (length s-str) 0)
                                (char= (char s-str (1- (length s-str))) #\%)
                                (try-parse-num (subseq s-str 0 (1- (length s-str))))))
                    (l-val (and (> (length l-str) 0)
                                (char= (char l-str (1- (length l-str))) #\%)
                                (try-parse-num (subseq l-str 0 (1- (length l-str))))))
                    (a (if (= n 4)
                           (let ((av (parse-alpha (elt parts 3))))
                             (when av (clamp (round4 av) 0.0 1.0)))
                           1.0)))
               (if (and h-val s-val l-val a)
                   (multiple-value-bind (r g b)
                       (hsl-to-rgb (mod h-val 360) (clamp s-val 0 100) (clamp l-val 0 100))
                     (list (clamp r 0 255)
                           (clamp g 0 255)
                           (clamp b 0 255)
                           (float a)))
                   :invalid))))
         ;; HWB/HWBA parser (CSS Color 4): hwb(H W% B%[ / A]).  W = whiteness,
         ;; B = blackness.  If W+B>=100% the colour is the grey W/(W+B); otherwise
         ;; the pure hue is scaled by (1-W-B) and lifted by W.
         (parse-hwb ()
           (let* ((open (position #\( s))
                  (close (when open (position #\) s :start open)))
                  (content (and open (subseq s (1+ open) (or close (length s)))))
                  (parts (and content (split-comps content)))
                  (n (length parts)))
             (unless (or (= n 3) (= n 4)) (return-from parse-hwb :invalid))
             (let* ((h-str (elt parts 0))
                    (h-len (length h-str))
                    (h-clean (if (and (>= h-len 3) (string= (subseq h-str (- h-len 3)) "deg"))
                                 (subseq h-str 0 (- h-len 3)) h-str))
                    (h-val (try-parse-num h-clean))
                    (w-str (elt parts 1)) (b-str (elt parts 2))
                    (w-val (and (> (length w-str) 0) (char= (char w-str (1- (length w-str))) #\%)
                                (try-parse-num (subseq w-str 0 (1- (length w-str))))))
                    (b-val (and (> (length b-str) 0) (char= (char b-str (1- (length b-str))) #\%)
                                (try-parse-num (subseq b-str 0 (1- (length b-str))))))
                    (a (if (= n 4)
                           (let ((av (parse-alpha (elt parts 3))))
                             (when av (clamp (round4 av) 0.0 1.0)))
                           1.0)))
               (if (and h-val w-val b-val a)
                   (let ((w (/ (clamp w-val 0 100) 100.0)) (bl (/ (clamp b-val 0 100) 100.0)))
                     (flet ((mk (r g b) (list (clamp (round r) 0 255) (clamp (round g) 0 255)
                                              (clamp (round b) 0 255) (float a))))
                       (if (>= (+ w bl) 1.0)
                           (let ((g (round (* 255 (/ w (+ w bl))))))
                             (mk g g g))
                           (multiple-value-bind (r g b) (hsl-to-rgb (mod h-val 360) 100 50)
                             (let ((scale (- 1.0 w bl)))
                               (mk (* 255 (+ (* (/ r 255.0) scale) w))
                                   (* 255 (+ (* (/ g 255.0) scale) w))
                                   (* 255 (+ (* (/ b 255.0) scale) w))))))))
                   :invalid)))))
      ;; Main dispatch
      (cond
        ((string= s "transparent")
         (list 0 0 0 0.0))
        ((gethash s *named-colors*)
         (let ((rgb (gethash s *named-colors*)))
           (list (first rgb) (second rgb) (third rgb) 1.0)))
        ((and (> len 0) (char= (char s 0) #\#))
         (parse-hex))
        ;; CSS Color 5 relative color: rgb(from C ...) / hsl(from C ...).
        ((and (>= len 8) (string= (subseq s 0 8) "rgb(from")) (parse-relative-color s :rgb))
        ((and (>= len 9) (string= (subseq s 0 9) "rgba(from")) (parse-relative-color s :rgb))
        ((and (>= len 8) (string= (subseq s 0 8) "hsl(from")) (parse-relative-color s :hsl))
        ((and (>= len 9) (string= (subseq s 0 9) "hsla(from")) (parse-relative-color s :hsl))
        ((and (> len 4) (or (string= (subseq s 0 5) "rgba(")
                            (string= (subseq s 0 4) "rgb(")))
         (parse-rgb))
        ((and (> len 4) (or (string= (subseq s 0 5) "hsla(")
                            (string= (subseq s 0 4) "hsl(")))
         (parse-hsl))
        ((and (>= len 4) (string= (subseq s 0 4) "hwb("))
         (parse-hwb))
        ;; CSS Color 4 device-independent color functions.
        ((and (>= len 6) (string= (subseq s 0 6) "oklch(")) (parse-lab-like s :oklch))
        ((and (>= len 6) (string= (subseq s 0 6) "oklab(")) (parse-lab-like s :oklab))
        ((and (>= len 4) (string= (subseq s 0 4) "lab(")) (parse-lab-like s :lab))
        ((and (>= len 4) (string= (subseq s 0 4) "lch(")) (parse-lab-like s :lch))
        ;; CSS Color 5 color-mix().
        ((and (>= len 10) (string= (subseq s 0 10) "color-mix(")) (parse-color-mix s))
        (t :invalid)))))