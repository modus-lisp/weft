  ...
  (let ((fs (cstyle-font-size cs)))
    (macrolet ((len (&optional auto) `(resolve-len value fs ,auto)))
      (cond
        ...
        ((string= prop "margin") (apply-box value fs cs #'(setf cstyle-margin-top) #'(setf cstyle-margin-right) #'(setf cstyle-margin-bottom) #'(setf cstyle-margin-left)))
        ...
        ((string= prop "margin-right") (let ((v (len))) (when v (setf (cstyle-margin-right cs) v))))
        ((string= prop "margin-bottom") (let ((v (len))) (when v (setf (cstyle-margin-bottom cs) v))))
        ((string= prop "margin-left") (let ((v (len))) (when v (setf (cstyle-margin-left cs) v))))
