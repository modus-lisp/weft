         ...
         (body-start (tk name)
           ...
           (cond
             ...
             ((equal name "li")
              ...
              ;; scan: close an enclosing <li>, stopping at special (except address/div/p)
              (loop for node in open
                    for nm = (dnode-name node)
                    do (cond ((equal nm "li")
                              (gen-implied "li")
                              (loop while (and open (not (equal (top-name) "li"))) do (pop open))
                              (when open (pop open)) (return))
                             ((and (member nm *special* :test #'equal)
                                   (not (member nm '("address" "div" "p") :test #'equal))) (return))))
              (when (in-scope "p" *button-scope*) (close-p))
             ...
             ((member name '("dd" "dt") :test #'equal)
              ...
              (loop for node in open
                    for nm = (dnode-name node)
                    do (cond ((member nm '("dd" "dt") :test #'equal)
                              (gen-implied nm)
                              (loop while (and open (not (equal (top-name) nm))) do (pop open))
                              (when open (pop open)) (return))
                             ((and (member nm *special* :test #'equal)
                                   (not (member nm '("address" "div" "p") :test #'equal))) (return))))
              (when (in-scope "p" *button-scope*) (close-p))
               ...
               ((eq ty :start-tag)
                (let ((name (tok-name tk)))
                  (cond
                    ...
                    ((equal name "caption")
                     (clear-to '("table" "html")) (insert-element "caption" (tok-attrs tk))
                     (push-marker) (switch :in-caption))
                    ((equal name "colgroup")
               ...
               (t ;; flush: foster-parent if any non-whitespace, else plain-insert
                (let ((text (apply #'concatenate 'string (nreverse pending))))
                  (if (some (lambda (c) (not (member c *ws*))) text)
                      (progn (setf fostering t) (reconstruct-afe) (insert-text text) (setf fostering nil))
                      (insert-text text)))
             ...
             (cond
               ((eq ty :start-tag)
                (let ((name (tok-name tk)))
                  (cond
                    ((member name '("td" "th") :test #'equal)
                     (clear-to '("tr" "html")) (insert-element name (tok-attrs tk))
                     (push-marker) (switch :in-cell))
                  ...
                  (cond
                    ((member name '("td" "th") :test #'equal)
                     (when (in-table-scope name)
                       (gen-implied)
                       (loop while (and open (not (equal (top-name) name))) do (pop open))
                       (when open (pop open)) (clear-afe-to-marker) (switch :in-row)))
             ...
             (cond
               ((and (eq ty :end-tag) (equal (tok-name tk) "caption"))
                (when (in-table-scope "caption")
                  ...
                  (when open (pop open)) (clear-afe-to-marker) (switch :in-table)))
               ((and (eq ty :start-tag)
                ...
                (when (in-table-scope "caption")
                  ...
                  (when open (pop open)) (clear-afe-to-marker) (switch :in-table) (reproc)))
               ((eq ty :eof) (return))
               (t (case ty
