;;;; forms-textarea.lisp — HTMLElement IDL surface for <textarea> (WPT swarm unit).
;;;; Self-registering into the element-proto hook (see core.lisp).  The installer
;;;; runs on the shared element prototype; each accessor must gate on the node's
;;;; tag ((string= (h:dnode-name (n this)) "textarea")) so it only acts on <textarea>.
(in-package #:weft.script)

(defun install-forms-textarea (ctx ep)
  (declare (ignorable ctx ep))
  (macrolet ((n (this) `(require-node ctx ,this)))
    ;; TODO (swarm): install the <textarea> IDL surface (gate each accessor on the tag).
    (macrolet ((%noop () nil)) (%noop))))

(register-element-proto-extension :textarea #'install-forms-textarea)
