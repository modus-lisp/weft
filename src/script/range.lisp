...
(defun install-range (ctx)
  (let* ((realm (context-realm ctx))
    ...
    (install-selection ctx rp)))

(defun make-range-object (ctx rp rg)
  "A Range JS object backed by boundary-point struct RG."
  (let ((obj (js:make-object :proto rp)))
    (setf (gethash obj (context-ranges ctx)) rg)
    obj))

(defun install-selection (ctx rp)
  "A minimal Selection (DOM Selection API): a single stored Range with the
anchor/focus accessors, addRange/getRangeAt/removeAllRanges/removeRange, and the
collapse/selectAllChildren mutators.  getSelection() on window and document both
return this one live object."
  (let* ((realm (context-realm ctx))
         (op (js:eval-script realm "Object.prototype"))
         (sel (js:make-object :proto op))
         (ranges '()))                      ; 0 or 1 Range JS objects
    (labels ((cur () (first ranges))
             (cur-rg () (let ((r (cur))) (and r (gethash r (context-ranges ctx)))))
             (set-rg (rg) (setf ranges (list (make-range-object ctx rp rg))))
             (meth (name arity fn) (js:put sel name (js:native-function realm name fn arity)
                                           :enumerable nil))
             (getr (name fn) (js:put-accessor sel name
                               :get (js:native-function realm (concatenate 'string "get " name)
                                      (lambda (this ig) (declare (ignore this ig)) (funcall fn)) 0)
                               :enumerable t :configurable t)))
      (getr "rangeCount" (lambda () (num (length ranges))))
      (getr "isCollapsed" (lambda () (let ((rg (cur-rg))) (jbool (or (null rg) (rg-collapsed-p rg))))))
      (getr "type" (lambda () (let ((rg (cur-rg)))
                                (cond ((null rg) "None") ((rg-collapsed-p rg) "Caret") (t "Range")))))
      (getr "anchorNode" (lambda () (let ((rg (cur-rg))) (if rg (wrap ctx (rg-sc rg)) js:*null*))))
      (getr "anchorOffset" (lambda () (let ((rg (cur-rg))) (num (if rg (rg-so rg) 0)))))
      (getr "focusNode" (lambda () (let ((rg (cur-rg))) (if rg (wrap ctx (rg-ec rg)) js:*null*))))
      (getr "focusOffset" (lambda () (let ((rg (cur-rg))) (num (if rg (rg-eo rg) 0)))))
      (meth "getRangeAt" 1 (lambda (this a) (declare (ignore this))
                             (let ((i (int-arg a 0)))
                               (unless (< -1 i (length ranges))
                                 (throw-dom ctx "IndexSizeError" 1 "index out of range"))
                               (nth i ranges))))
      (meth "addRange" 1 (lambda (this a) (declare (ignore this))
                           (when (and (null ranges) (rg-of ctx (arg a 0)))
                             (setf ranges (list (arg a 0))))
                           js:*undefined*))
      (meth "removeRange" 1 (lambda (this a) (declare (ignore this))
                              (setf ranges (remove (arg a 0) ranges)) js:*undefined*))
      (meth "removeAllRanges" 0 (lambda (this a) (declare (ignore this a))
                                  (setf ranges '()) js:*undefined*))
      (meth "empty" 0 (lambda (this a) (declare (ignore this a)) (setf ranges '()) js:*undefined*))
      (meth "collapse" 2 (lambda (this a) (declare (ignore this))
                           (if (nullish (arg a 0)) (setf ranges '())
                               (let ((n (require-node ctx (arg a 0))) (o (int-arg a 1)))
                                 (set-rg (make-rg :sc n :so o :ec n :eo o))))
                           js:*undefined*))
      (meth "setPosition" 2 (lambda (this a) (declare (ignore this))
                              (if (nullish (arg a 0)) (setf ranges '())
                                  (let ((n (require-node ctx (arg a 0))) (o (int-arg a 1)))
                                    (set-rg (make-rg :sc n :so o :ec n :eo o))))
                              js:*undefined*))
      (meth "collapseToStart" 0 (lambda (this a) (declare (ignore this a))
                                  (let ((rg (cur-rg)))
                                    (when rg (set-rg (make-rg :sc (rg-sc rg) :so (rg-so rg)
                                                             :ec (rg-sc rg) :eo (rg-so rg)))))
                                  js:*undefined*))
      (meth "collapseToEnd" 0 (lambda (this a) (declare (ignore this a))
                                (let ((rg (cur-rg)))
                                  (when rg (set-rg (make-rg :sc (rg-ec rg) :so (rg-eo rg)
                                                           :ec (rg-ec rg) :eo (rg-eo rg)))))
                                js:*undefined*))
      (meth "extend" 2 (lambda (this a) (declare (ignore this))
                         (let ((rg (cur-rg)) (n (require-node ctx (arg a 0))) (o (int-arg a 1)))
                           (if rg (set-rg (make-rg :sc (rg-sc rg) :so (rg-so rg) :ec n :eo o))
                               (set-rg (make-rg :sc n :so o :ec n :eo o))))
                         js:*undefined*))
      (meth "selectAllChildren" 1 (lambda (this a) (declare (ignore this))
                                    (let ((n (require-node ctx (arg a 0))))
                                      (set-rg (make-rg :sc n :so 0 :ec n :eo (node-len n))))
                                    js:*undefined*))
      (meth "containsNode" 2 (lambda (this a) (declare (ignore this))
                               (let ((rg (cur-rg)) (n (node-of ctx (arg a 0))))
                                 (jbool (and rg n (eq (h:dnode-parent n) (rg-sc rg)))))))
      (meth "toString" 0 (lambda (this a) (declare (ignore this a))
                           (let ((rg (cur-rg))) (if rg (rg-string rg) ""))))
      (js:put sel (js:eval-script realm "Symbol.toStringTag") "Selection"
              :enumerable nil :writable nil :configurable t)
      ;; getSelection() on both window and document return this singleton.
      (let ((getfn (js:native-function realm "getSelection"
                     (lambda (this a) (declare (ignore this a)) sel) 0)))
        (js:define-global realm "getSelection" getfn)
        (js:put (proto ctx :window) "getSelection" getfn :enumerable nil :writable t :configurable t)
        (js:put (proto ctx :document) "getSelection" getfn :enumerable nil :writable t :configurable t))
      sel)))
