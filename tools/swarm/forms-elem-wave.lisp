...
(defun keep-cmd (job)
  "What the oracle runs when it sees a new best: snapshot the WHOLE worktree as
   ...
   The pinned .sexp tables are EXCLUDED.  They are regenerated by every scoring
   run, so canon's copies change the moment the first arm is kept — and from
    ...
    (let* ((job (getf winner :job))
      ...
      (note "  -> ~a touches: ~a" (job-id job)
            (or (sh "git -C ~a apply --numstat ~a 2>/dev/null | awk '{print $3}' | paste -sd,"
                    *src* patch)
                "?"))
      ;; Snapshot the tree BEFORE touching it.  `git apply --3way' is only
      ;; atomic when it refuses outright: when it conflicts it WRITES the
      ;; conflicted state — markers in the files, stages in the index — and
      ;; exits 1.  Left there, those markers are what the next unit gets scored
      ;; against, so one conflicting arm makes every arm after it score 0 and
      ;; "revert" work that was never at fault.  That cascade is what happened
      ;; in wave 7.  The snapshot holds the earlier units' kept patches, which
      ;; is why the recovery below can afford a hard reset.
          ...
          (multiple-value-bind (out code err) (sh "git -C ~a apply --3way ~a" *src* patch)
            ...
            (if (not (zerop code))
                (progn
                  ...
                  (restore)
                  (note "  -> tree restored to the state before ~a; later units unaffected"
                        (job-id job)))
                (let ((after (total-of (canon-score unit))))
                  (cond ((and after before (> after before) (scripting-dom-ok-p *src*))
                        ...
                        (t
                         ;; Restore from the snapshot rather than reverse-applying:
                         ;; a reverse apply can conflict too, and then it leaves the
                         ;; same wreckage the forward one would have.
                         (restore)
